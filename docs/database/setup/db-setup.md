...
Once you have defined your [Domain Structure](/database/setup/data-model), run `sbt moleculeGen` and compiled your project, you're ready to use molecules to transact and fetch data from databases. You can also configure [custom database column properties](/database/setup/db-column-props) to customize SQL column types for your attributes.
...
Dependencies in `build.sbt`:
```scala
"org.scalamolecule" %% "molecule-db-postgres" % "0.30.0",
// already imports
